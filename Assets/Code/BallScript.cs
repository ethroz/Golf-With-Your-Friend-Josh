...
public class BallScript : MonoBehaviour
{
    ...
    private const long resetMilliseconds = 1000;
    ...
    private void FixedUpdate()
    {
        ...
        if (velocity.sqrMagnitude != 0.0f)
            velocity -= 0.5f * FluidDensity * velocity.sqrMagnitude * dragCoefficient * CSArea * velocity.normalized * UnityEngine.Time.fixedDeltaTime;
        ...
        if (Grounded()) // if the ball is on the same sloped ground as last frame
        {
            ...
            if (rHit.collider.gameObject.tag == "Terrain" || Dead || (rHit.collider.GetComponentInParent<LevelScript>() != null &&
                rHit.collider.GetComponentInParent<LevelScript>().LevelIndex != GameManagerScript.LevelIndex))
            {
                Respawn = true;
            }
            ...
            velocity -= rollingResistanceCoefficient * Gravity.magnitude * Mathf.Cos(gravityAngle) * velocity.normalized * UnityEngine.Time.fixedDeltaTime//;
                * (velocity.sqrMagnitude / 100.0f + 1.0f); // apply roational resistance dependent on speed squared to slow down the ball at fast speeds
            ...
            if (velocity.magnitude < 0.1f)
            {
                if (!timer.IsRunning)
                    timer.Restart();
                if (timer.ElapsedMilliseconds > resetMilliseconds)
                {
                    velocity = new Vector3();
                    NotMoving = true;
                    return;
            ...
            while (Time > 0.0f) // collision loop
            {
                if (WillCollide()) // if the ball is going to collide with something
                {
                    ...
                    fDistanceTravelled += d0;
                    Distance -= d0; // update the distance travelled by the ball
                    ...
                    if (angle < fShallowAngle) // if the next collision is at a shallow enough angle then assume it is rounded ground
                    {
                        ...
                        Distance = velocity.magnitude * Time; // calculate the new remaining distance before end of frametime
                ...
                else
                {
                    ...
                    fDistanceTravelled += Distance;
                    break;
                }
            }
            angularSpeed = fDistanceTravelled / Radius * Mathf.Rad2Deg;
        }
        else
        {
            ...
            while (Time > 0.0f) // collision loop
            {
                if (WillCollide()) // if the ball is going to collide with something
                {
                    ...
                    if (prevPos == position) // avoid getting stuck in the loop
                        break;
                    trail.AddPosition(position); // add the new position to the trail renderer
                    float d0 = (position - prevPos).magnitude; // distance travelled
                    Distance -= d0;
